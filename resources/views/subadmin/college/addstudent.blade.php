            ...
            <div class="card bg-secondary shadow border-0">
                <div class="card-body px-lg-5 py-lg-5">
                    <div class="text-center text-muted mb-4">
                        <a href="{{ route('school.student', ['id' => $college->college_id]) }}" class="btn btn-sm btn-primary float-right d-inline"><i class="fa fa-angle-double-left"></i> {{ trans('messages.back') }}</a>
                        <small>Add Student to {{$college->name}}</small>
                    </div>
                    <form method="POST" action="{{ route('school.newstudent', [$college->college_id]) }}">
                    {{ csrf_field() }}
                        ...
                        <div class="form-group">
                            <div class="input-group input-group-alternative mb-3">
                                ...
                                <input id="username" type="text" placeholder="{{ __('Registration Number') }}" class="form-control @error('username') is-invalid @enderror" name="username" value="{{ old('username') }}" required autocomplete="username" autofocus>
